...
const createConverter = (config) => {
  ...
  const convert = (json) => {
    const content = json.map((file) => {
      ...
      return rows.map((row) => (config.namespace ? `    ${row}` : row)).join("\n");
    });

    const filteredContent = content.filter((x) => x.length > 0);

    if (config.namespace) {
      return [`export {};\n\ndeclare ${config.namespace} {`, ...filteredContent, "}"].join("\n");
    } else {
  ...
  const convertEnum = (enum_, filename) => {
    ...
    const getEnumStringValue = (value) => (config.camelCaseEnums ? camelcase(value) : value);
